...
INSTALL_EXEC_MODE ?= 0755

ifeq ($(INSTALL_OWNERSHIP),y)
INSTALL_ARGS_OWNERSHIP = -g $(INSTALL_GROUP) -o $(INSTALL_OWNER)
endif

ifeq ($(INSTALL_PERMS),y)
INSTALL_ARGS_DIR_PERMS = -m $(INSTALL_DIR_MODE)
INSTALL_ARGS_FILE_PERMS = -m $(INSTALL_FILE_MODE)
INSTALL_ARGS_EXEC_PERMS = -m $(INSTALL_EXEC_MODE)
endif

#############################################################################
# Local overrides (for development purposes)
#############################################################################
...
# No examples written at this stage
#COMPONENTS += examples
endif
